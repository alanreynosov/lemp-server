...
bash 'update_assets' do
  code <<-EOH
    ...
    aws s3 sync /srv/www/current s3://#{app[:environment][:assets_bucket ]}/ --exclude="*.php" --exclude="*.html" --exclude="*.gz" --exclude=".git/*" --exclude=".htaccess" --exclude="*.txt" --exclude=".gitignore"
  EOH
end